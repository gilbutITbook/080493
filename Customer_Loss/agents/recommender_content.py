# =============================================
# agents/recommender_content.py — 콘텐츠 기반 추천 + gRPC 서버
# =============================================
...
class ContentRecommenderAgent:
    ...
    def _ensure_core(self, all_feats: List[Dict[str, Any]]) -> None:
        ...
        # 상품 간 유사도 계산: 상품 간 유사도 행렬 생성 (콘텐츠 기반 추천에 필요)
    ...
    async def recommend(self, cid: str, all_feats: List[Dict[str, Any]], top_n: int = 5) -> List[Dict[str, Any]]:
        """콘텐츠 기반 추천 실행 - 고객이 구매한 상품과 유사한 상품 추천"""
        self._ensure_core(all_feats)  # 행렬 구축 확인
        # RecommenderCore의 recommend_content 메서드를 호출하여 추천 상품 리스트 획득
        # 이 메서드는 고객이 구매한 상품과 유사한 상품을 찾아 반환
        pairs = self.core.recommend_content(cid, top_n)
        ...
        for name, score in pairs:
            ...
            out.append(
                {
                    "상품명": name,
                    "추천점수": float(round(score, 3)),  # 유사도 점수 (소수점 3자리)
                    "카테고리": info.get("category", "기타"),
                    "가격": info.get("price", 0),
                    "설명": info.get("description", ""),
                    "추천이유": "보유 상품과의 콘텐츠 유사도 기반 추천",
                }
            )
        return out


class ContentRecommendService(pbg.ContentRecommendServiceServicer):
    """gRPC 서비스 구현 - 콘텐츠 기반 추천 서비스"""
    
    def __init__(
        self,
        feature_endpoint: str = "127.0.0.1:6101",
    ) -> None:
        self.agent = ContentRecommenderAgent(feature_endpoint)  # 콘텐츠 기반 추천 에이전트 인스턴스
    ...
    def RecommendContent(self, request: pb.RecommendReq, context) -> pb.JsonList:  # type: ignore[override]
        """gRPC 메서드 구현 - 콘텐츠 기반 추천 요청 처리"""
...
async def serve(host: str = "127.0.0.1", port: int = 6105) -> None:
    """gRPC 서버 시작 및 실행 - 콘텐츠 기반 추천 서비스를 제공하는 독립 서버"""
    server = grpc.aio.server()  # 비동기 gRPC 서버 인스턴스 생성
    # ContentRecommendService를 서버에 등록 (gRPC 서비스 구현체 연결)
    pbg.add_ContentRecommendServiceServicer_to_server(ContentRecommendService(), server)
    # 서버가 수신할 포트 설정 
    server.add_insecure_port(f"{host}:{port}")
    await server.start()  # 서버 시작
    print(f"[grpc] ContentRecommendService running at {host}:{port}")
    # 서버가 종료될 때까지 대기 
...
if __name__ == "__main__":
    """직접 실행 시 gRPC 서버 시작"""
    # 이 파일을 직접 실행하면 콘텐츠 기반 추천 서비스가 포트 6105에서 시작됨
    asyncio.run(serve())
